...
| REG-016 | WhatsApp opens wrong variant on Samsung | Tapping "WhatsApp" in the chooser opened WhatsApp Business instead | Samsung One UI's modified ActivityManagerService ignores the `package=` parameter in `intent://` URIs and resolves to the system-default WhatsApp handler | `_openInPackage()` now uses `MethodChannel` → `Intent.setPackage()` in Kotlin; unit tests in `test/unit/core/utils/whatsapp_launcher_test.dart` verify NANP + KSA normalization paths |
| REG-017 | Version name collision | `2.2.10+99` and `2.3.0+99` shared the same versionCode (99); pre-commit hook produced a lower semantic version than the previous build | Pre-commit hook bumped PATCH from `2.3.0` incorrectly producing `2.2.10` instead of `2.3.1` | Fixed by manually setting `2.3.1+100`; add version monotonicity assertion to pre-commit hook |
| REG-018 | File corruption via large patches | `tech_dashboard_screen.dart` was structurally corrupted (orphaned widget fragments, ~500 cascade errors) by repeated large multi-hunk patches during AI-assisted editing | Large apply-patch operations on a 1000+ line widget file drifted against a shifting tree; partial hunks landed mid-class | Small-edit protocol in `.claude/rules/code-quality.md` (≤1 logical block per edit on >800-line files, `get_errors` after every edit); restore-from-HEAD playbook; per-edit error check is mandatory before the next edit |
| REG-019 | Sign-out state isolation | New providers added to session scope were not added to the sign-out invalidation list — `monthlyTechnicianStatsProvider`, `unreadMonthClosureProvider`, `monthClosuresProvider`, `latestMonthClosureProvider` all leaked state to next logged-in user | Sign-out list in `auth_providers.dart` was not updated when new session-scoped providers were added in the v14 session | Every new session-scoped provider MUST be added to `signOut()` invalidation list in `auth_providers.dart`; v15 audit gate checks provider count delta |
| REG-020 | Hardcoded Firestore document IDs | `month_closures` string literal used directly in `month_closure_repository.dart` instead of `AppConstants` | `AppConstants.monthClosuresDocId` was not created when the month-closure feature was added | `AppConstants.monthClosuresDocId = 'month_closures'` added; all doc ID strings must be constants in AppConstants; `audit.yml` hardcoded-string gate catches future violations |
